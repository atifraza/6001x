...
class WordTrigger(Trigger):
    ...
    def isWordIn(self, text):
        text_clean = self.stripPunctuation(text.lower())
        words = text_clean.split(" ")
        if self.word_trigger.lower() in words:
            return True
        else:
            return False
...
class PhraseTrigger(Trigger):
    ...
    def evaluate(self, story):
        result = False
        if self.trigger_phrase in story.getTitle():
            result = True
        elif self.trigger_phrase in story.getSubject():
            result = True
        elif self.trigger_phrase in story.getSummary():
            result = True
        return result
...
def makeTrigger(triggerMap, triggerType, params, name):
    ...
    if triggerType == "TITLE":
        trigger = TitleTrigger(joinedParams)
    elif triggerType == "SUBJECT":
        trigger = SubjectTrigger(joinedParams)
    elif triggerType == "SUMMARY":
        trigger = SummaryTrigger(joinedParams)
    elif triggerType == "NOT":
        trigger = NotTrigger(triggerMap[joinedParams])
        ...
        trigger = PhraseTrigger(joinedParams)
...
def main_thread(master):
    ...
    try:
        ...
        triggerlist = readTriggerConfig("D:\\work\\python\\6001x\\ProblemSet7\\triggers.txt")
